...
export function useAgentOperations() {
    ...
    try {
      if (agent.status === 'offline') {
        ...
        const agentConfig = existingConfig ? {
          ...
          systemPrompt: existingConfig.systemPrompt || `You are ${existingConfig.name}, a ${existingConfig.role} agent.`,
  ...
  }, [activeProjectId, availableConfigs, projects, sendClaudeMessage, updateAgentSessionId])

  /**
   * Remove agent from team
   * Kills process and removes from UI
   */
  const removeAgentFromTeam = useCallback(async (
    agentId: string,
    agentName: string
  ): Promise<AgentOperationResult> => {
   ...
   */
  const addAgentsToProject = useCallback(async (
    agentIds: string[]
  ): Promise<AgentOperationResult> => {
    if (!activeProjectId) {
      return { success: false, error: 'No active project' }
  ...
  }, [activeProjectId, availableConfigs, processManager])
