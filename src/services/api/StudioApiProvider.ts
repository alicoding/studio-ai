...
export class StudioApiProvider extends BaseApiClient implements StudioProvider {
  ...
  constructor(config?: ProviderConfig) {
    ...
    const finalConfig = config || ApiConfigService.getInstance().getProviderConfig('studio') || {
      name: 'studio',
      baseUrl: '/api',
  ...
  readonly agents = {
    ...
    abort: (id: string, projectId: string) =>
      this.post<void>(`agents/${id}/abort`, { projectId }),
  ...
  readonly teams = {
    ...
    clone: (id: string, name?: string) =>
      this.post<Team>(`teams/${id}/clone`, { name }),
  ...
  readonly projects = {
    ...
    addTag: (id: string, tag: string) =>
      this.post<Project>(`projects/${id}/tags`, { tag }),
    ...
    getSessionMessages: (id: string, sessionId: string, options?: { cursor?: string; limit?: number }) => {
      const queryParams = options ? {
        ...(options.cursor && { cursor: options.cursor }),
        ...(options.limit && { limit: options.limit.toString() })
      } : undefined
      return this.get<PaginatedResponse<Message>>(`projects/${id}/sessions/${sessionId}/messages`, queryParams)
    },
  ...
  readonly messages = {
    ...
    mention: (data: MentionData) =>
      this.post<Message>('messages/mention', data),
    
    sendSystem: (data: SystemMessageData) =>
      this.post<Message>('messages/system', data),
  ...
  readonly system = {
    ...
    cleanupZombies: () => this.post<{ cleaned: number }>('system/cleanup-zombies'),
    
    health: () => this.get<HealthStatus>('system/health'),
    
    detectCommand: (command: string) =>
      this.post<{ isCommand: boolean; command?: string; args?: string[] }>('system/detect-command', { command }),
    
    checkPath: (path: string) => this.post<{ exists: boolean; type?: 'file' | 'directory' }>('system/check-path', { path })
  }
  ...
  readonly settings = {
    ...
    updateHooks: (hooks: Record<string, HookConfig>) => this.put<Hook[]>('settings/hooks', { hooks }),
    
    testHook: (hook: HookConfig) => this.post<{ success: boolean; output?: string; error?: string }>('settings/hooks/test', { hook })
  }
  ...
  readonly diagnostics = {
    ...
    check: (data: DiagnosticData) =>
      this.post<DiagnosticInfo>('diagnostics/check', data)
  }
  ...
  readonly screenshots = {
    capture: (data: ScreenshotData) =>
      this.post<Screenshot>('screenshot', data)
  }
   ...
   */
  readonly search = {
    index: (data: SearchIndexData) =>
      this.post<SearchIndexStats>('search/index', data),
