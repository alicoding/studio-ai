...
router.get('/namespaces', async (req, res) => {
  try {
    const db = getDb()
    const rows = await db.selectDistinct({ namespace: storageTable.namespace }).from(storageTable)
...
router.get('/items', async (req, res) => {
  try {
    ...
    if (namespace && namespace !== 'all' && search) {
      ...
      rows = await db.select().from(storageTable).limit(Number(limit))
    }
...
router.get('/item/:namespace/:key', async (req: Request<StorageParams>, res: Response) => {
  try {
...
router.post(
  '/item/:namespace/:key',
  async (req: Request<StorageParams, unknown, StorageSetBody>, res: Response) => {
    try {
      const { namespace, key } = req.params
      ...
      const config: StorageConfig = {
        namespace,
        type: type as 'config' | 'state' | 'secret' | 'cache' | 'session',
        encrypt,
      }
...
router.delete('/item/:namespace/:key', async (req: Request<StorageParams>, res: Response) => {
  try {
    const { namespace, key } = req.params
    const db = getDb()
    ...
    await db
      .delete(storageTable)
      .where(and(eq(storageTable.key, key), eq(storageTable.namespace, namespace)))
