...
router.post('/', async (req, res) => {
  try {
    const { path: screenshotPath, interactive = false, captureArea = false, base64Data, width, height } = req.body
    ...
    if (platform === 'darwin') {
      ...
      try {
        await execAsync(command)
        res.json({ success: true, path: screenshotPath, note: 'Screenshot in clipboard, please save manually' })
      } catch (error) {
        res.status(500).json({ error: 'Failed to take screenshot' })
      }
    } else {
