...
export class UnifiedAgentConfigService {
    ...
    async updateConfig(id, request) {
        ...
        if (request.tools !== undefined) {
            ...
            if (Array.isArray(request.tools) && request.tools.length > 0) {
                if (typeof request.tools[0] === 'string') {
                    ...
                    updates.tools = this.toolPermissionService.serializePermissions(request.tools);
                }
            }
            else {
        ...
        if (request.temperature !== undefined)
            updates.temperature = String(request.temperature);
        const db = this.getDatabase();
    ...
    async assignRole(request) {
        ...
        await db
            ...
            .where(and(eq(agentRoleAssignments.projectId, request.projectId), eq(agentRoleAssignments.role, request.role)));
     ...
     */
    async getProjectRoleAssignments(projectId) {
        // Check cache first
     ...
     */
    async getBatchProjectRoleAssignments(projectIds) {
     ...
     */
    async getRoleAssignment(projectId, role) {
        const db = this.getDatabase();
        const assignments = await db
            .select()
            .from(agentRoleAssignments)
     ...
     */
    async removeRoleAssignment(projectId, role) {
        const db = this.getDatabase();
        await db
            .delete(agentRoleAssignments)
            .where(and(eq(agentRoleAssignments.projectId, projectId), eq(agentRoleAssignments.role, role)));
     ...
     */
    mapRoleAssignmentFromDb(dbRecord) {
        return {
            id: dbRecord.id,
     ...
     */
    async getEffectiveToolPermissions(projectId, role) {
        // Get role assignment
        const assignments = await this.getProjectRoleAssignments(projectId);
     ...
     */
    async updateToolPermissions(agentConfigId, tools) {
     ...
     */
    async applyPermissionPreset(agentConfigId, presetName) {
        const config = await this.getConfig(agentConfigId);
        if (!config) {
            return null;
        }
        // Get current tool names
        const currentTools = this.toolPermissionService.getEnabledToolNames(this.toolPermissionService.parseTools(config.tools));
        // Apply preset
...
//# sourceMappingURL=UnifiedAgentConfigService.js.map